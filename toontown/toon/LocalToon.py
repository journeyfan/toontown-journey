...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def initInterface(self):
        ...
        if base.wantAchievements:
            ...
            self.book.addPage(
                self.achievementsPage, pageName=TTLocalizer.AchievementsPageTitle
            )
        if base.wantKarts:
            self.addKartPage()
    ...
    def localPresentPie(self, time):
        ...
        self.sendUpdate(
            "presentPie", [pos[0], pos[1], pos[2], hpr[0] % 360.0, timestamp32]
        )
    ...
    def refreshOnscreenButtons(self):
        ...
        if self.moveFurnitureButtonObscured <= 0:
            if self.furnitureManager != None and self.furnitureDirector == self.doId:
                ...
                if self.__lerpFurnitureButton:
                    self.__lerpFurnitureButton.finish()
                self.__lerpFurnitureButton = self.__furnitureGui.posHprScaleInterval(
                    1.0,
                    pos=Point3(0.115, 0.0, -0.66),
    ...
    def setSleepAutoReply(self, fromId):
        ...
        if isinstance(av, (DistributedToon.DistributedToon, FriendHandle)):
            base.localAvatar.setSystemMessage(
                0, TTLocalizer.SleepAutoReply % av.getName(), WTToontownBoardingGroup
            )
        elif av:
