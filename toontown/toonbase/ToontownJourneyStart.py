...
notify = directNotify.newCategory('Toontown Journey Client Start')
notify.setInfo(True)
...
notify.info('Starting Toontown Journey...')
if launcher.isDummy():
    http = HTTPClient()
else:
    http = launcher.http
...
notify.info('Setting the default Toontown Journey font...')
...
if base.musicManagerIsValid:
    ...
    if music:
        ...
        music.setVolume(0.9)
        music.play()
    notify.info('Loading the default Toontown Journey GUI sounds...')
    DirectGuiGlobals.setDefaultRolloverSound(base.loadSfx('phase_3/audio/sfx/GUI_rollover.ogg'))
    DirectGuiGlobals.setDefaultClickSound(base.loadSfx('phase_3/audio/sfx/GUI_create_toon_fwd.ogg'))
else:
    music = None
